//  const site_url = "http://127.0.0.1:8000/";
 const site_url = "https://dispatchaapp.com.ng/";
$.ajaxSetup({
    headers: {
     ...
     if(text.length > 1){
        $.ajax({
            ...
            result.forEach(element => {
                if(element.hasOwnProperty('pDate')){


                    var row = `<tr>
                    <td class="d-none">${element.order_id}</td>
                    <td data-bs-toggle="modal"
                      data-bs-target="#exLargeModal2"
                      id="details2">Pickup Name: <b>${element.name}  </b>
                    Pickup Address: <b>${element.address} </b>  <br>
                    <small>Pickup Phone Number: <b>${element.phoneNumber}  </b> - ${element.created_at} </small>
                    </td>
                    </tr>`;
                    data += row;

                }else if(element.hasOwnProperty("dDate")){
                    ...
                    <td data-bs-toggle="modal"
                      data-bs-target="#exLargeModal2"
                      id="details2">Delivery Name: <b>${element.name}  </b>
                    Delivery Address: <b>${element.address} </b>  <br>
                    <small>Delivery Phone Number: <b>${element.phoneNumber}  </b> - ${element.created_at} </small>
                    </td>
                    </tr>`;
                    data += row;

                }else if (element.hasOwnProperty("orderNumber")) {

                      var row = `<tr>
                    <td class="d-none">${element.id}</td>
                    <td data-bs-toggle="modal"
                      data-bs-target="#exLargeModal2"
                      id="details2">Order Number: <b>${element.orderNumber}  </b>
                    Pickup Address: <b>${element.pickup.address} </b> Delivery Address: <b>${element.deliver_to.address} </b> <br>
                    <small> Pickup Phone Number: <b>${element.pickup.phoneNumber}  </b> Delivery Phone Number: <b>${element.deliver_to.phoneNumber}  </b> - ${element.created_at} </small>
                    </td>
                    </tr>`;
                      data += row;
